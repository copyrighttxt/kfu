...
#define KFU_TRAYMENU_CFU		WM_APP + 9
...
#define KFU_FCS_120				KFU_FCS_FIRST + 4
#define KFU_FCS_144				KFU_FCS_FIRST + 5
#define KFU_FCS_240				KFU_FCS_FIRST + 6
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case KFU_TRAYICON:
	{
		if (lParam == WM_RBUTTONDOWN || lParam == WM_LBUTTONDOWN)
		{
			...
			AppendMenu(popup, MF_STRING | (Settings::UnlockClient ? MF_CHECKED : 0), KFU_TRAYMENU_CLIENT, "Unlock Roblox Player");
			AppendMenu(popup, MF_STRING | (Settings::UnlockStudio ? MF_CHECKED : 0), KFU_TRAYMENU_STUDIO, "Unlock Roblox Studio");
			AppendMenu(popup, MF_STRING | (Settings::CheckForUpdates ? MF_CHECKED : 0), KFU_TRAYMENU_CFU, "Check for Updates");
			...
			if (result != 0)
			{
				...
				case KFU_TRAYMENU_CFU:
					Settings::CheckForUpdates = !Settings::CheckForUpdates;
					CheckMenuItem(popup, KFU_TRAYMENU_CFU, Settings::CheckForUpdates ? MF_CHECKED : MF_UNCHECKED);
					break;
...
#ifdef _WIN64
	SetConsoleTitleA("Kireon FPS Unlocker " KFU_VERSION " (64-bit) Console");
#else
	SetConsoleTitleA("Kireon FPS Unlocker " KFU_VERSION " (32-bit) Console");
#endif
...
{	
	...
	UI::Window = CreateWindow("KFUClass", "Kireon FPS Unlocker", 0, 0, 0, 0, 0, NULL, NULL, instance, NULL);
	if (!UI::Window)
		return 0;
	...
	strcpy_s(NotifyIconData.szTip, "Kireon FPS Unlocker");
