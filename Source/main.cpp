...
std::vector<HANDLE> GetRobloxProcesses(bool include_client = true, bool include_studio = true)
{
	...
	if (include_client)
	{
		for (HANDLE handle : ProcUtil::GetProcessesByImageName("KireonPlayerBeta.exe")) result.emplace_back(handle);
...
const void *FindTaskScheduler(HANDLE process, const char **error = nullptr)
{
	try
	{
		...
		while (true)
		{
			...
			{
				if (error) *error = "Failed to get process base! Restart Kireon FPS Unlocker or, if you are on a 64-bit operating system, make sure you are using the 64-bit version of Kireon FPS Unlocker.";
				return nullptr;
			}
		}
...
bool CheckRunning()
{
	SingletonMutex = CreateMutexA(NULL, FALSE, "KFUMutex");
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	else
	{
		if (CheckRunning())
		{
			MessageBoxA(NULL, "Kireon FPS Unlocker is already running", "Error", MB_OK);
		}
		else
		{
